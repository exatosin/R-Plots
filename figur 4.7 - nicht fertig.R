...
pred3 = predict(fit, newdata = new_data, se.fit = T)

pred3

select = c(3,10,30)
other_women = dat[select,]
other_women$x
pred = predict(fit, newdata = other_women, se.fit = T)

pred

pred$fit
mittelwert2 = pred$fit[2]
standardabweichung = pred$residual.scale

mein_y= 50:200
meine_density = dnorm(mein_y, mean = mittelwert2, sd = standardabweichung)

plot(mein_y, meine_density, type = "l")
