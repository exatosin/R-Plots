...
p1 <- ggplot(data = data.frame(A = c(-1, 2)), aes(A)) + 
  ylab("") +
  xlab("") +
  ...
  geom_segment(aes(x = -3.9, y = 0, xend = 2.9, yend = 0),         # draw x-axis
               size = achsendicke) +
  geom_text(x=2.8, y=-0.03, label=expression(Y), size=6) +  # y axis label
  geom_text(x=-3.5, y=0.54, label="P(Y)", size=6) +         # "P(Y)" x axis label
  geom_text(x=-3.3, y=p_Value_low, label="p", size=6) +     # "p" x axis label
  geom_text(x=-3.5, y=p_value_high, label="1-p", size=6) +  # "p-1" x axis label
  geom_segment(aes(x = -2.9, y = p_Value_low,               # dashed vertical line to "p"
                xend = -1, yend = p_Value_low), 
               size = liniendicke, linetype="dashed") + 
  geom_segment(aes(x = -2.9, y = p_value_high,               # dashed vertical line to "1-p"
                   xend = 1, yend = p_value_high), 
               size = liniendicke, linetype="dashed") + 
  # draw white square on top of right end of too long x-axis (work-around)
  geom_rect(aes(xmin=-4.0, xmax=-3.1, ymin=-0.02,  ymax=0.02), 
            fill="white", alpha=1) +
